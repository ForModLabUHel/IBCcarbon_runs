  ...
  if(procInSample){
    if(is.null(initSoilC)){
      if(outType=="uncRun"){
        ...
        if(identical(landClassX,1:3)) load(paste0(path_initiSoilC, "/initSoilC/forCent",r_no,"/initSoilC_",sampleID,"_LandClass1to3.rdata"))
        if(identical(landClassX,1:2)) load(paste0(path_initiSoilC, "/initSoilC/forCent",r_no,"/initSoilC_",sampleID,"_LandClass1to2.rdata"))
        if(identical(landClassX,1)) load(paste0(path_initiSoilC, "/initSoilC/forCent",r_no,"/initSoilC_",sampleID,"_LandClass1.rdata"))
      }
    }
...
###set parameters to increase rotation length of 25% (end)
  ...
  if(harvScen=="Base" & harvInten =="Base" & initilizeSoil & rcps=="CurrClim"){
    ...
    if(outType!="testRun" | forceSaveInitSoil){
      if(!outType %in% c("uncRun","uncSeg")){
        if(identical(landClassX,1:3)) save(initSoilC,file=paste0(path_initiSoilC, "/initSoilC/forCent",r_no,"/initSoilC_",sampleID,"_LandClass1to3.rdata"))
        if(identical(landClassX,1:2)) save(initSoilC,file=paste0(path_initiSoilC, "/initSoilC/forCent",r_no,"/initSoilC_",sampleID,"_LandClass1to2.rdata"))
        if(identical(landClassX,1)) save(initSoilC,file=paste0(path_initiSoilC, "/initSoilC/forCent",r_no,"/initSoilC_",sampleID,"_LandClass1.rdata"))
