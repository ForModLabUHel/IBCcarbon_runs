...
if(sampleFromAgeClass){
  source_url("https://raw.githubusercontent.com/ForModLabUHel/IBCcarbon_runs/master/finRuns/Rsrc/sampleFromAgeClass.r")
}
...
if(harvScen=="Base"){
  ...
  cons=region$ClCut+1
  cons[which(cons==2)]=0
  areas <- data.table(segID=region$siteInfo[,1],area=region$areas,cons=cons)
...
if(harvScen=="protect"){
  ...
  cons=region$ClCut+1
  cons[which(cons==2)]=0
  areasProtect <- data.table(segID=region$siteInfo[,1],area=region$areas,cons=cons)
