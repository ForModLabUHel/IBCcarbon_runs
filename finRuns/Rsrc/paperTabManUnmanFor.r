...
###calculate areas by region
areaCountry <- sum(meanRegion[harScen=="Base" & harvInten=="Base" & year==1]$area)
areaCountryManFor <- sum(meanRegion[harScen=="Base" & harvInten=="Base" & year==1& cons==0]$area)
areaCountryUnmanFor <- sum(meanRegion[harScen=="Base" & harvInten=="Base" & year==1& cons==1]$area)
areaCountry.protect <- sum(meanRegion[harScen=="protect" & harvInten=="Base" & year==1]$area)
areaCountryManFor.protect <- sum(meanRegion[harScen=="protect" & harvInten=="Base" & year==1& cons==0]$area)# * areaRegion/areaRegion.protect
areaCountryUnmanFor.protect <- sum(meanRegion[harScen=="protect" & harvInten=="Base" & year==1& cons==1]$area)# * areaRegion/areaRegion.protect

###add areas to table
protScen <- c("protect", "protectTapio")
meanRegion[harScen %in% protScen & cons==0,areaCountry:=areaCountryManFor.protect]
meanRegion[harScen %in% protScen & cons==1,areaCountry:=areaCountryUnmanFor.protect]
meanRegion[!harScen %in% protScen & cons==0,areaCountry:=areaCountryManFor]
meanRegion[!harScen %in% protScen & cons==1,areaCountry:=areaCountryUnmanFor]
meanRegion[,countryAreaWeight:=area/areaCountry]

###calculate weights from areas
meanRegionWeighted <- meanRegion
meanRegionWeighted[, vars] <- 
  meanRegionWeighted[ ,lapply(.SD, `*`, countryAreaWeight), .SDcols = vars]

###calculate country averages
meanCountry <- 
  meanRegionWeighted[ ,lapply(.SD, sum,na.rm=T),
                      .SDcols = vars,by=.(harScen,year,harvInten,cons)]
